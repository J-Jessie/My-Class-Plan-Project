...
<?php
require_role([ROLE_TIMETABLER, ROLE_ADMIN]);
$pdo = getPDO($CONFIG);

// Handle create allocation (demo-only; in production INSERT into 'allocations' and log to 'audit_log')
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["create_allocation"])) {
  $payload = [
  ...
  // In demo, store in session list
  $_SESSION["allocations_demo"][] = $payload;
  $_SESSION["audit_demo"][] = date('Y-m-d H:i:s')." - Created allocation: ".json_encode($payload);
  flash("ok","Allocation added (demo).");
  header("Location: ".$_SERVER["PHP_SELF"]);
  exit;
}

// Build events from DB or demo
$events = fetchAllocations($pdo);
if (!empty($_SESSION["allocations_demo"])) {
  // Merge demo entries into events for display
  $courses = fetchCourses($pdo);
  $rooms = fetchRooms($pdo);
  $lects = fetchLecturers($pdo);
  $cIndex = []; foreach ($courses as $c) $cIndex[$c["id"]] = $c;
  $rIndex = []; foreach ($rooms as $r) $rIndex[$r["id"]] = $r;
  $lIndex = []; foreach ($lects as $l) $lIndex[$l["id"]] = $l;
  foreach ($_SESSION["allocations_demo"] as $a) {
    $events[] = [
      "day"=>$a["day"],
      "time"=>$a["time"],
      "course_code"=>$cIndex[$a["course_id"]]["code"] ?? "C???",
      "course_id"=>$a["course_id"],
      "room_code"=>$rIndex[$a["room_id"]]["code"] ?? "R???",
      "room_id"=>$a["room_id"],
      "lecturer_name"=>$lIndex[$a["lecturer_id"]]["name"] ?? "Unknown",
...
echo "<h1 class='mb-3'>Timetabler Dashboard</h1>";
renderFlash();
?>
<div class="row g-4">
  <div class="col-lg-5">
    <div class="card shadow-sm h-100">
      <div class="card-body">
        <h5 class="card-title">Create / Assign Class</h5>
        <form method="post">
          <input type="hidden" name="create_allocation" value="1"/>
          <div class="mb-2">
            <label class="form-label">Course</label>
            <select name="course_id" class="form-select" required>
              <option value="">Select…</option>
              <?php foreach (fetchCourses($pdo) as $c): ?>
          ...
          <div class="mb-2">
            ...
            <select name="lecturer_id" class="form-select" required>
              <option value="">Select…</option>
              <?php foreach (fetchLecturers($pdo) as $l): ?>
          ...
          <div class="mb-2">
            ...
            <select name="room_id" class="form-select" required>
              <option value="">Select…</option>
              <?php foreach (fetchRooms($pdo) as $r): ?>
                <option value="<?= $r["id"] ?>"><?= htmlspecialchars($r["code"])." (".$r["capacity"].")" ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="row g-2">
            <div class="col-6">
              ...
              <select name="day" class="form-select">
                <?php foreach ($CONFIG["days"] as $d): ?><option value="<?= $d ?>"><?= $d ?></option><?php endforeach; ?>
              </select>
            </div>
            <div class="col-6">
              <label class="form-label">Start Time</label>
              <select name="time" class="form-select">
                <?php foreach ($CONFIG["slots"] as $t): ?><option value="<?= $t ?>"><?= $t ?></option><?php endforeach; ?>
              </select>
            </div>
          </div>
          <div class="mt-2">
            <label class="form-label">Group</label>
            <input type="text" name="group_label" class="form-control" placeholder="e.g., Y1, BIT-2"/>
          </div>
          <button class="btn btn-primary mt-3 w-100">Add to Timetable</button>
        </form>
      </div>
    </div>
  </div>
    ...
    <div class="card shadow-sm h-100">
      <div class="card-body">
        <div class="d-flex justify-content-between align-items-center">
          <h5 class="card-title mb-0">Timetable (All)</h5>
          <span class="badge <?= empty($conflicts)?'text-bg-success':'text-bg-danger' ?>">
            <?= empty($conflicts)?'No conflicts detected':(count($conflicts).' conflict(s)') ?>
          </span>
        </div>
        <div class="mt-2">
          <?php echo renderTimetableGrid($events, $CONFIG); ?>
        </div>
        <?php if (!empty($conflicts)): ?>
          <div class="alert alert-danger mt-3">
            <div class="fw-semibold">Conflicts</div>
            <ul class="mb-0">
              <?php foreach ($conflicts as [$i,$j]): ?>
                <li>Collision between <code><?= htmlspecialchars($events[$i]["course_code"]) ?></code> and <code><?= htmlspecialchars($events[$j]["course_code"]) ?></code> on <?= htmlspecialchars($events[$i]["day"]) ?> at <?= htmlspecialchars($events[$i]["time"]) ?> (room/lecturer clash).</li>
              <?php endforeach; ?>
            </ul>
          </div>
        <?php endif; ?>
...
<div class="card shadow-sm mt-4">
  <div class="card-body">
    ...
    <ul class="list-group list-group-flush">
      <?php $logs = $_SESSION["audit_demo"] ?? []; if (empty($logs)): ?>
        <li class="list-group-item text-muted">No audit entries yet.</li>
      <?php else: foreach (array_reverse($logs) as $line): ?>
...
<!--
Suggested tables:
CREATE TABLE allocations (id INT PK AI, course_id INT, lecturer_id INT, room_id INT, day ENUM('Mon','Tue','Wed','Thu','Fri','Sat'), time TIME, group_label VARCHAR(50), UNIQUE KEY uniq_slot (day, time, room_id));
CREATE TABLE audit_log (id INT PK AI, actor_id INT, action VARCHAR(50), payload JSON, created_at DATETIME DEFAULT CURRENT_TIMESTAMP);
-->
