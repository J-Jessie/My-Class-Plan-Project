...
// --- DEMO notifications (or fetch from DB 'notifications' table for the student) ---
$notifications = [
  ["title"=>"Timetable Published","body"=>"Semester 1 timetable is now live.","time"=>"Today"],
  ["title"=>"Room Change","body"=>"CSC101 on Thu 11:00 moved to B1-01.","time"=>"Yesterday"],
];
$group = $_GET["group"] ?? "Y1";
$events = fetchAllocations($pdo, ["student_group"=>$group]);
echo "<h1 class='mb-3'>Student Dashboard</h1>";
renderFlash();
?>
<div class="row g-4">
  <div class="col-lg-4">
    <div class="card shadow-sm h-100">
      <div class="card-body">
        ...
        <div><b>Name:</b> <?= htmlspecialchars($_SESSION["name"]) ?></div>
        <div><b>Group:</b> <?= htmlspecialchars($group) ?></div>
        <div class="mt-3">
          <form method="get" class="row gy-2 gx-2 align-items-end">
            <div class="col-8">
              <label class="form-label">Change Group</label>
              <input type="text" name="group" class="form-control" placeholder="e.g., Y1, BIT-2" value="<?= htmlspecialchars($group) ?>"/>
            </div>
            <div class="col-4"><button class="btn btn-primary w-100">View</button></div>
          </form>
        </div>
      </div>
    </div>
    ...
    <div class="card shadow-sm h-100">
      <div class="card-body">
        <h5 class="card-title d-flex align-items-center justify-content-between">
          ...
          <a href="?group=<?= urlencode($group) ?>&export=pdf" class="btn btn-outline-secondary btn-sm">Export</a>
        </h5>
        <?php
          echo renderTimetableGrid($events, $CONFIG, ["group_label"=>$group]);
        ?>
      </div>
    </div>
  </div>
        ...
        <ul class="list-group list-group-flush">
          <?php foreach ($notifications as $n): ?>
            <li class="list-group-item d-flex justify-content-between align-items-start">
              ...
              <span class="badge text-bg-light"><?= htmlspecialchars($n["time"]) ?></span>
            </li>
          <?php endforeach; ?>
        </ul>
      </div>
...
CREATE TABLE notifications (id INT PK AI, user_id INT, title VARCHAR(120), body TEXT, created_at DATETIME);
-->
